...
void setup() {
  ...
  Serial1.readBytes(buf_Version, 17);
  Serial.println("Software Version:");
  for (int i=0; i<17; i++) {
    Serial.print(buf_Version[i], HEX);
    Serial.print("-");
  }
  Serial.println("");
  ...
  Serial1.readBytes(buf_Serial, 16);
  Serial.println("Device Serial:");
  for (int i=0; i<16; i++) {
    Serial.print(buf_Serial[i], HEX);
    Serial.print("-");
  }
  Serial.println("");
  ...
  Serial1.readBytes(buf_Cont, 6);
  Serial.println("Continuous Set:");
  for (int i=0; i<6; i++) {
    Serial.print(buf_Cont[i], HEX);
    Serial.print("-");
  }
  Serial.println("");
  ...
  Serial1.readBytes(buf_Meas, 6);
  Serial.println("Measurement Set:");
  for (int i=0; i<6; i++) {
    Serial.print(buf_Meas[i], HEX);
    Serial.print("-");
  }
  Serial.println("");
...
void loop() {
  if (Serial1.available()) {
    ...
    Serial.print(":");
    Serial.print(buf_Read[3]*256 + buf_Read[4]);
    Serial.println(" ppm");
